...
### 2，注意事项
* map 是<span style="color: red;">**并发读写不安全**</span>的数据结构，在多个 goroutine 同时读写 map，是需要对整个 map 单独加互斥锁或读写锁；对整个 map 添加互斥锁时，如果 map 中的数据量非常大，那么锁的粒度也很大，可能会导致性能瓶颈，如果要使用并发安全的 map，可以使用 [sync.Map](https://aliang-codes.github.io/my-blogs/pages/6855f1/)；
